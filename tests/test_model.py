...
test_input = {
    ...
    "token_class_labels": torch.Tensor([[0, 3, 4, 0, 0, 3, 1, 0]]),
}


def test_base_model():
    output = model(
        **{k: v for k, v in test_input.items() if k in ["input_ids", "attention_mask"]}
    )

    batch_size, sequence_length = test_input["input_ids"].shape
    embedding_size = 768
...
def test_composite_model():
    num_token_classes = int(test_input["token_class_labels"].max() + 1)
    composite_model = CompositeModel(model, num_token_classes=num_token_classes)

    output = composite_model(**test_input)
